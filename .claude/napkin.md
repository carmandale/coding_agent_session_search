...
| 2026-04-03 | Session | amp + opencode connectors were re-scanning 33+0 conversations every cycle with zero new data, contributing to WAL write load that triggered frankensqlite 26of FK mismatch. | Stub connectors that haven't had sessions in 60+ days. Check `br ready` for candidate connectors by querying last session date. |
| 2026-04-03 | Session | franken_insert_message had no FK violation handling. FrankenError::ForeignKeyViolation from 26of MVCC snapshot mismatch crashed the watcher. | Catch ForeignKeyViolation in franken_insert_message, return Ok(None); callers use let-else continue to skip. Affected messages retry next cycle. |
| 2026-04-03 | Session | Upstream `Cargo.toml` had an active local `[patch]` to `../frankensqlite/...`, so CI/worktrees without that sibling checkout fail before dependency resolution. | For sync branches, comment local frankensqlite `[patch]` and attest effective resolved rev with `cargo metadata` + explicit `cargo tree -i <spec>` for ambiguous multi-rev graphs. |
| 2026-04-03 | Session | Disabling Amp/OpenCode via connector stubs changes indexer tests that assume Amp ingests fixtures; `cargo test --lib` gains 6 expected Amp-path failures vs upstream baseline. | In upstream-sync receipts, classify test deltas as expected behavior change when connectors are intentionally stubbed; compare against upstream baseline under the same dependency posture (patch disabled). |
| 2026-04-04 | Session | `/code-verify` preflight "test files found" only counted `tests/` path changes and underreported inline unit-test edits in `src/*`. Reviewers flagged this as evidence mismatch even when targeted tests were correct. | In verification bundles/receipts, explicitly list inline test-bearing `src` files and tie them to executed targeted test commands/results. |
| 2026-04-04 | Session | Receipt test claims drifted from actual saved log artifacts after test command changes (6→7), causing reviewer distrust even though code was correct. | When test command/count changes, regenerate preflight output and update receipt artifact paths in the same pass (`codeverify-targeted-finalN`, `codeverify-preflightN`). |
| 2026-04-03 | Session | frankensqlite MVCC page-lock deadlock: watcher wedged on txn_id=61 incrementing page_id every 60s. cass health unhealthy. | Restart watcher with launchctl kickstart -k. This is a 26of symptom; no code fix available until upstream fixes frankensqlite. |
