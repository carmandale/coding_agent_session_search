...
2026-05-15 14:53 | — | claude-code/opus-4.7 | /codex-implement | T16 verification result — PR #90 fix landed correctly (179/179 tests pass, Cargo.toml [patch] verified via cargo metadata, release build clean, binary installed at ~/.local/bin/cass.real) BUT does not bring peak RSS under 8 GB on cass workload. Post-fix run: pi count frozen at 36, RSS 55+ GB at 3 min. Pre-fix vs post-fix RSS curves comparable (post is actually higher because the cloning-related fragmented free was hiding the steady-state cost). Root cause: fsqlite_ext_fts5 keeps the entire InvertedIndex in memory (~30 GB on this corpus). Per-savepoint clone (which PR #90 eliminates) is real cost but NOT the dominant cost. Spec 014 acceptance #2 structurally unsatisfiable. Cargo.toml reverted, previous binary restored, watcher reloaded. Three follow-up paths E1/E2/E3 documented in notes/T16-verification-result.md; user decision required.
